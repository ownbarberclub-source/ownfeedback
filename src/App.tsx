...
export default function App() {
  ...
  const loadData = async () => {
    ...
    const { data: dbBarbers, error: barbersError } = await supabase.from('previa_barbers').select('*').eq('is_hidden_crm', false).eq('is_active', true).order('name');
              ...
              <form onSubmit={handleSubmitEval} className="space-y-8">
                <div className="space-y-8 bg-zinc-900/40 p-12 rounded-3xl border border-zinc-900 shadow-2xl backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-10">
                     <div className="space-y-4"><label className="text-[10px] uppercase font-bold text-zinc-500 tracking-widest text-center block">Já é Assinante?</label><div className="flex gap-4"><button type="button" onClick={() => setNewEval({...newEval, isSubscriber: true})} className={`flex-1 py-4 rounded-xl border font-black text-xs transition-all ${newEval.isSubscriber === true ? 'bg-brand border-brand text-white' : 'bg-zinc-950 border-zinc-800 text-zinc-700'}`}>SIM</button><button type="button" onClick={() => setNewEval({...newEval, isSubscriber: false})} className={`flex-1 py-4 rounded-xl border font-black text-xs transition-all ${newEval.isSubscriber === false ? 'bg-zinc-800 border-zinc-700 text-white' : 'bg-zinc-950 border-zinc-800 text-zinc-700'}`}>NÃO</button></div></div>
